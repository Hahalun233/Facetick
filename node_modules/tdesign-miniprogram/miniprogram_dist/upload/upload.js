...
let Upload = class Upload extends SuperComponent {
    ...
    chooseImg() {
        ...
        wx.chooseMedia(Object.assign(Object.assign({ count: max === 0 ? 9 : max, mediaType: ['image'] }, config), { success: (res) => {
                const files = [];
    ...
    chooseVideo() {
        ...
        wx.chooseMedia(Object.assign(Object.assign({ mediaType: ['video'] }, config), { success: (res) => {
                const files = [];
                res.tempFiles.forEach((temp) => {
                    if (sizeLimit && temp.size > sizeLimit) {
